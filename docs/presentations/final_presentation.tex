...
\begin{frame}{Evolution: Midterm to Final}
    \begin{columns}[T]
        ...
        \vspace{0.3cm}
        \textbf{5G Authorization Application:}
        \begin{itemize}
            \item Same cryptographic primitive applies to LMF authorization
            \item 5 parties: Judicial, Law Enforcement, Network Security, Privacy Officer, Auditor
            \item Require 3-of-5 approval to decrypt location requests
        \end{itemize}
    \end{columns}
\end{frame}
...
\begin{frame}{System Architecture: 5G Authorization Framework}
    \begin{center}
        \begin{tikzpicture}[scale=0.75, every node/.style={scale=0.75}]
            ...
            % LMF with lock
            \node[draw, rectangle, fill=red!20, minimum width=1.5cm, minimum height=0.8cm] (lmf) at (9,0) {LMF};
            \node[draw, circle, fill=yellow!40, minimum size=0.5cm] at (9.8, 0.5) {\tiny Lock};
            
            % Authorization Parties
            \node[draw, circle, fill=green!20, minimum size=0.9cm] (p1) at (12,2.5) {\tiny Judicial};
            \node[draw, circle, fill=green!20, minimum size=0.9cm] (p2) at (13.5,1.5) {\tiny Law Enf.};
            \node[draw, circle, fill=green!20, minimum size=0.9cm] (p3) at (12,0) {\tiny Net Sec};
            \node[draw, circle, fill=green!20, minimum size=0.9cm] (p4) at (13.5,-1.5) {\tiny Privacy};
            \node[draw, circle, fill=green!20, minimum size=0.9cm] (p5) at (12,-2.5) {\tiny Auditor};
            
            % Flows
            \draw[->, thick] (ue) -- node[above] {\tiny Positioning} (gnb);
            \draw[->, thick] (gnb) -- (amf);
            \draw[->, thick] (amf) -- node[above] {\tiny Loc Req} (lmf);
            
            % Authorization
            \draw[->, thick, dashed, red] (lmf) -- node[above, sloped] {\tiny Share 1} (p1);
            \draw[->, thick, dashed, red] (lmf) -- node[above, sloped] {\tiny Share 2} (p2);
            \draw[->, thick, dashed, red] (lmf) -- node[above] {\tiny Share 3} (p3);
            \draw[->, thick, dashed, red] (lmf) -- node[below, sloped] {\tiny Share 4} (p4);
            \draw[->, thick, dashed, red] (lmf) -- node[below, sloped] {\tiny Share 5} (p5);
            
            % Legend
            \node[text width=4cm, align=left] at (3, -3) {\scriptsize \textbf{Setup:} LMF access key split into 5 shares};
...
\begin{frame}{Experimental Metrics \& Baseline}
...
\begin{frame}[fragile]{Execution Results}
...
\begin{frame}{Security Analysis Results}
    ...
    \begin{itemize}
        ...
        \item ✓ Threshold security (3-of-5) enforced
        \item ✓ Private key exists only during decryption ($<$5ms)
        \item ✓ Information-theoretic security (Shamir's SSS)
        \item ✓ Correct Pre-Master Secret recovery
    \end{itemize}
\end{frame}
...
\begin{frame}{Design Challenges \& Solutions}
    \textbf{Design Choices \& Architectural Decisions}
    ...
    \textbf{Challenge 3: Implementation approach?}
    \begin{itemize}
        \item \textbf{Initial:} Complex 600-line implementation with extensive error handling → Segmentation faults
        \item \textbf{Final:} Simplified 350-line focused implementation → Working prototype
        \item \textbf{Lesson:} Start simple, validate core functionality, then add complexity
    \end{itemize}
...
\begin{frame}{Future Work: 5G Integration \& Extensions}
